...
uint8_t Statistics(FILE *FileToRead) {
    ...
    char msg_low[]  = "\e[31mToo Low\e[m";
    char msg_high[] = "\e[31mToo High\e[m";
    char msg_good[] = "\e[32mGood\e[m";
            ...
            if (((DisplayType)&(1<<0))==TYPE_ASCII) {
                if (DisplaySection[Cursor]!=CheckPattern[Cursor]) {
                    ...
                    if (((DisplayType)&(1<<2))==DISPLAY_FAIL) {
                        printf ("Section: %s: read '%s' !   mismatch pattern '%s' ! \e[31mFAIL\e[m\n"
                            , SectionName
                            , DisplaySection
                            , CheckPattern);
                    }
                    ...
                    if (((DisplayType)&(1<<3))==DISPLAY_GOOD) {
                        printf ("Section: %s: read '%s' !   match pattern '%s' ! \e[32mGOOD\e[m\n"
                            , SectionName
                            , DisplaySection
                            , CheckPattern);
                    }
...
uint8_t CheckFilledData (FILE *FileToRead, uint32_t *PercentCheck){
    ...
    while (SectionFilled[Cursor].name!=NULL) {
        ...
        if (strcmp(SectionFilled[Cursor].Pattern,"xx")==0) {
            ...
            if (!Status3) {
                printf ("Succesfully checked '%s' from '0x%08X' to '0x%08X' is empty (00 or FF) ! \e[32mGOOD\e[m\n"
                    , SectionFilled[Cursor].name
                    , SectionFilled[Cursor].Offset
                    , SectionFilled[Cursor].Offset+SectionFilled[Cursor].Size);
            }
            else {
                printf ("Some error occured when checking '%s' from '0x%08X' to '0x%08X' ! \e[31mFAIL\e[m\n"
                    , SectionFilled[Cursor].name
                    , SectionFilled[Cursor].Offset
                    , SectionFilled[Cursor].Offset+SectionFilled[Cursor].Size);
            }
            ...
            if (!Status3) {
                printf ("Succesfully checked '%s' from '0x%08X' to '0x%08X' is full of '0x%s' ! \e[32mGOOD\e[m\n"
                    , SectionFilled[Cursor].name
                    , SectionFilled[Cursor].Offset
                    , SectionFilled[Cursor].Offset+SectionFilled[Cursor].Size
                    , SectionFilled[Cursor].Pattern);
...
//    char    DisplaySection[DATA_BUFFER_SIZE]={0};
    ...
    if (NbFileTOCros0<NB_MAX_FILE_ROS) {
        ...
        printf ("Found %d files in the TOC of ros0, max is %d ! \e[31mFAIL\e[m\n" , NbFileTOCros0 , NB_MAX_FILE_ROS);
        return EXIT_FAILURE;
    }
    ...
    if (NbFileTOCros1<NB_MAX_FILE_ROS) {
        ...
        printf ("Found %d files in the TOC of ros1, max is %d ! \e[31mFAIL\e[m\n" , NbFileTOCros1 , NB_MAX_FILE_ROS);
        return EXIT_FAILURE;
    }
    ...
    if (MD5Found == IS_BROKEN) {
        printf ("\ttrvk_prg0's header is broken ! \e[31mFAIL\e[m\n");
        Status = EXIT_FAILURE;
        MD5Found = NOT_FOUND;
    }
    else if (MD5Found == NOT_BROKEN) {
        printf ("\ttrvk_prg unknown but its header is correct for info its size is '0x%04X'\n",trvk_prg0Size);
        Status = EXIT_FAILURE;
        MD5Found = NOT_FOUND;
    }
    else if (MD5Found == NOT_FOUND) {
        printf ("\ttrvk_prg unknown ! \e[31mFAIL\e[m\n");
        Status = EXIT_FAILURE;
    }
    else {
        printf ("\ttrvk_prg for FW :'%s' item '%d' in the list ! \e[32mGOOD\e[m\n",trvk_prg_MD5[MD5Found].Version, MD5Found);
        MD5Found = NOT_FOUND;
    }
    ...
    if (MD5Found == IS_BROKEN) {
        printf ("\ttrvk_prg1's header is broken ! \e[31mFAIL\e[m\n");
        Status = EXIT_FAILURE;
        MD5Found = NOT_FOUND;
    }
    else if (MD5Found == NOT_BROKEN) {
    ...
    if (MD5Found == IS_BROKEN) {
        printf ("\ttrvk_pkg0's header is broken ! \e[31mFAIL\e[m\n");
        Status = EXIT_FAILURE;
        MD5Found = NOT_FOUND;
    }
    else if (MD5Found == NOT_BROKEN) {
        printf ("\ttrvk_pkg unknown but its header is correct for info its size is '0x%04X'\n",trvk_pkg0Size);
        Status = EXIT_FAILURE;
        MD5Found = NOT_FOUND;
    }
    else if (MD5Found == NOT_FOUND) {
        printf ("\ttrvk_pkg unknown ! \e[31mFAIL\e[m\n");
        Status = EXIT_FAILURE;
    }
    else {
        printf ("\ttrvk_pkg for FW :'%s' item '%d' in the list ! \e[32mGOOD\e[m\n",trvk_pkg_MD5[MD5Found].Version,MD5Found);
        MD5Found = NOT_FOUND;
    }
    ...
    if (MD5Found == IS_BROKEN) {
        printf ("\ttrvk_pkg1's header is broken ! \e[31mFAIL\e[m\n");
        Status = EXIT_FAILURE;
        MD5Found = NOT_FOUND;
    }
    else if (MD5Found == NOT_BROKEN) {
    ...
    while (SectionRos[Cursor].name!=NULL) {
            ...
            if (Cursor<=NbFileTOCros0-1) {
                printf("SDK Version '%c.%c%c' | MD5 : '", ROS0SDKVersion[0],ROS0SDKVersion[2],ROS0SDKVersion[3]);
                }
            else {
                printf("SDK Version '%c.%c%c' | MD5 : '", ROS1SDKVersion[0],ROS1SDKVersion[2],ROS1SDKVersion[3]);
            }

            printMD5(MD5result);
            printf ("' for '%s'\n",SectionRos[Cursor].name);
...
//      {"cvtrm"           , 0xECxxxx                                         , 0x0xxxxx           , TYPE_HEX, 1, "00"}, // need to investigate
        ...
        //{"CELL_EXTNOR_AREA", 0xF69400                                         , 0x0130             , TYPE_HEX, 1, pattern_zz},
        ...
        if (strcmp(SectionFilled[Cursor].Pattern,pattern_zz)==0) {
            for (Cursor2=0;Cursor2<SectionFilled[Cursor].Size;Cursor2+=0x4) {
                ...
                if (Status2) {
                    ...
                    , SectionFilled[Cursor].DisplayType
                    , Buffer);
                    printf ("Error at '0x%08X' found :'0x%08X' ! \e[31mFAIL\e[m\n", SectionFilled[Cursor].Offset+Cursor2, *Buffer);
                }
                Status3 |= Status2;
            }
            if (!Status3) {
                printf ("Succesfully checked '%s' from '0x%08X' to '0x%08X' is not empty ! \e[32mGOOD\e[m\n"
                    , SectionFilled[Cursor].name
                    , SectionFilled[Cursor].Offset
                    , SectionFilled[Cursor].Offset+SectionFilled[Cursor].Size);
            }
...
int main(int argc, char *argv[]) {
    ...
    printf("to give you a validation status of roughly 97%%!?\n");
    ...
    if (argc==2) {
        OptionType = OPTION_STATS + OPTION_CHECK_GENERIC + OPTION_CHECK_PERPS3 + OPTION_CHECK_FILLED + OPTION_CHECK_NOT_ZERO + OPTION_CHECK_PER_FW;
    }
    ...
    if (GlobalStatus){
        ...
        for (Cursor=1;Cursor<NBToReport;Cursor++) {
            if (GlobalReport[Cursor]){
                printf("\t\e[31mError in %s\e[m\n",ReportDetail[Cursor].ReportName);
                printf("\t\t-%s\n",ReportDetail[Cursor].ReportMsg);
            }
        }
        printf("\n");
        printf("\t!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
        printf("\t! \e[31mYOU FAIL come back later\e[m !\n");
        printf("\t!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    }
    else{
        //printf("\n");
        ...
        printf("\n");
        printf("\t!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
        printf("\t! \e[32mGOOD, same player, play again!?\e[sm !\n");
        printf("\t!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    }
